...
pub fn write_header<W>(writer: &mut W, header: DataFrameHeader) -> PunpunResult<()>
	where W: Write {
...
pub fn read_header<R>(reader: &mut R) -> PunpunResult<DataFrameHeader>
	where R: Read {
	...
	if opcode >= 8 {
		...
		if !flags.contains(FIN) {
			return Err(PunpunError::ProtocolError(
